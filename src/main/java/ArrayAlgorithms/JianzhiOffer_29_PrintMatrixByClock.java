...
public class JianzhiOffer_29_PrintMatrixByClock {
    ...
    public ArrayList<Integer> printMatrix2(int[][] matrix) {
         ...
         * 当行数为奇数时，如rows=5，start需要0,1,2；当rows=4时，start需要0,1；因此row<(rows+1)>>1或者row<<1 <rows
         */
        int startEnd = ((rows < cols ? rows : cols) + 1) >> 1;
        for (int start = 0; start < startEnd; start++) {
//        for (int start = 0; (start << 1) < rows && (start << 1) < cols; start++) {
            int rowEnd = rows - 1 - start;//下行行号
            int columnEnd = cols - 1 - start;//右列列号
            //打印上行
            for (int j = start; j <= columnEnd; j++) result.add(matrix[start][j]);
            //打印右列:至少需要两行
            // 从start+1开始至小于等于rowEnd限制了至少两行
            for (int i = start + 1; i <= rowEnd; i++) result.add(matrix[i][columnEnd]);

            //打印下行：//至少两行且至少两列才需要打印：
            // ①rowEnd > start控制了至少两行；
            // ②从columnEnd-1开始至大于等于start控制了至少两列<重点>
            //容易忽略&& rowEnd > start
            for (int j = columnEnd - 1; j >= start && rowEnd > start; j--) result.add(matrix[rowEnd][j]);

            //打印左列：至少三行两列:columnEnd > start控制了至少两列；从rowEnd-1开始至小于start控制了至少三行
            //容易忽略：&& columnEnd > start
            for (int i = rowEnd - 1; i > start && columnEnd > start; i--) result.add(matrix[i][start]);
        }
        return result;
    }
